...
class AdminDashboardAnalytics
{
    ...
    public static function activeCampaignsWithoutRecentDonations(int $days = 7, int $limit = 5): Collection
    {
        ...
        return Campaign::query()
            ...
            ->withCount(['donations as successful_donations_count' => fn (Builder $query) => $query->where('status', 'success')])
            ->where('status', Campaign::STATUS_ACTIVE)
            ->whereDoesntHave('donations', fn (Builder $query) => $query
                ->where('status', 'success')
    ...
    {
        return [
            'pending_campaign_reviews' => Campaign::query()->where('status', Campaign::STATUS_PENDING)->count(),
            'pending_campaigner_profiles' => CampaignerProfile::query()->where('status_verifikasi', 'menunggu')->count(),
    ...
    {
        return Donation::query()->whereBetween('created_at', [$start, $end]);
    }
