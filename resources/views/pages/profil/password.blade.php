<div x-show="tab === 'password'" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-cloak
    x-data="{ showCurrent: false, showNew: false, showConfirm: false }" x-init="$nextTick(() => lucide.createIcons())">

    <div class="mb-8">
        <h3 class="text-xl font-bold text-slate-800">Perbarui Kata Sandi</h3>
        <p class="text-sm text-slate-500">Pastikan akun Anda menggunakan kata sandi yang panjang dan acak untuk keamanan.
        </p>
    </div>
    @if (session('success'))
    ...
    @endif
    <form action="{{ route('profile.password.update') }}" method="POST" class="space-y-6 w-full">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-2">
            ...
            <div class="relative group">
                ...
                <button type="button" @click="showCurrent = !showCurrent; $nextTick(() => lucide.createIcons())"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <button type="button" @click="showNew = !showNew; $nextTick(() => lucide.createIcons())"
                        class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-600 transition-colors">
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    @error('password_confirmation')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    <button type="button" @click="showConfirm = !showConfirm; $nextTick(() => lucide.createIcons())"
                        class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-600 transition-colors">
        ...
        <div class="pt-6 border-t border-slate-100 flex flex-col sm:flex-row gap-3 justify-end">
            <button type="submit"
                class="px-10 py-3 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-200 active:scale-95 transition-all">
                Perbarui Kata Sandi
            </button>
        </div>
