...
@section('content')
    @php
        ...
        if ($user) {
            if (method_exists($user, 'isAdmin') && $user->isAdmin()) {
                $campaigns = Campaign::latest()->get(); // ADMIN → semua
            } else {
                $campaigns = Campaign::where('user_id', $user->id)->latest()->get(); // USER → miliknya
            }
        } else {
            $campaigns = collect();
        }
    ...
    <main x-data="{ showModal: false, showVerifyModal: false, isVerified: {{ $isVerified ? 'true' : 'false' }} }" class="min-h-screen bg-slate-50 pt-30 md:pt-32 pb-20">
            ...
            <div class="max-w-7xl mx-auto">
                @if (!$hasCampaign)
                    <div class="flex flex-col items-center text-center">
                        ...
                        <p class="text-slate-400 text-sm mb-10 max-w-xs">Kamu belum memiliki campaign yang sedang berjalan
                            saat
                            ini.</p>
                ...
                @else
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                        @foreach ($campaigns as $item)
                            @php
                                $percentage =
                                    $item->target_amount > 0 ? ($item->current_amount / $item->target_amount) * 100 : 0;
                                $days_left = \Carbon\Carbon::now()->diffInDays(
                                    \Carbon\Carbon::parse($item->end_date),
                                    false,
                                );
                                $days_left = $days_left > 0 ? floor($days_left) : 0;
                            @endphp
                            <x-galang-donasi-card :title="$item->title" :description="\Illuminate\Support\Str::limit(strip_tags($item->description), 80)" :raised="$item->current_amount" :goal="$item->target_amount"
                                :percentage="$percentage" :image="asset('storage/' . $item->image)" :donatur="rand(10, 200)" :days="$days_left" :status="$item->status"
                                :rejection-reason="$item->status === Campaign::STATUS_REJECTED ? $item->rejection_reason : null" :detail-url="$item->status === Campaign::STATUS_ACTIVE
                                    ? route('donation.detail', $item->slug)
                                    : \App\Filament\Admin\Resources\Campaigns\CampaignResource::getUrl('edit', [
                                        'record' => $item,
                                    ])" :edit-url="$item->status === Campaign::STATUS_REJECTED
                                    ? \App\Filament\Admin\Resources\Campaigns\CampaignResource::getUrl('edit', [
                                        'record' => $item,
                                    ])
                                    : null" />
